...
async def _report_worker(queue: asyncio.Queue, session: aiohttp.ClientSession):
    ...
    while True:
        ...
        try:
            async with session.post(REPORT_URL_BAK, json=[payload], timeout=aiohttp.ClientTimeout(total=10)) as resp:
                resp.raise_for_status()
                logging.info("[Reporter beta] 上报成功 状态码=%s", resp.status)
        except aiohttp.ClientResponseError as e:
            body = await e.response.text() if e.response else ""
            logging.error("[Reporter beta] 上报失败 HTTP=%s body=%s", e.status, body)
        except Exception as e:
            logging.error("[Reporter beta] 上报异常: %s", e)
